...
Next Steps: My aim for this project is to successfully fly a drone from pixels in simulation, this capability is in testing but due to limited access to sufficient compute progress on this is slow. To achieve this I must ensure my architecture is as optimised as possible, a point where the code in its current state is approaching. Another possible change is a more powerful VAE, this would come at the cost of greater compute needed in training and inference so will only be implemented if testing identifies the VAE as a bottleneck.

To run the CarRacer training loop use: 
python train_car_racer.py --config car_racer_config.yaml
